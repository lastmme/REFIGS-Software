...
def tag_sparse_peak_matches_and_generate_scores(matchLibTags, matchLibIntensities, matchQueTags, matchQueIntensities):
    ...
    return remove, np.array(returnMaccScores)
...
def collect_relevant_ppm_values(ppmMatches, maccScores, maccCutoff):
    ...
    return ppmMatches[maccScores >= maccCutoff]
...
class IdentificationSpectraMatcher:
    ...
    def compare_spectra(self, pooledLibSpectra: npt.NDArray, pooledQueSpectra: npt.NDArray, tolerance, idToDecoyDict):
        ...
        libMzs, libIntensities, libTags = (
            pooledLibSpectra[:, 0],
            pooledLibSpectra[:, 1],
            pooledLibSpectra[:, 2]
        )
        queMzs, queIntensities, queTags = (
            pooledQueSpectra[:, 0],
            pooledQueSpectra[:, 1],
            pooledQueSpectra[:, 2]
        )
        self.libraryTags, self.libraryIntensities, self.queryTags, self.queryIntensities, self.ppmMatches = smf.find_matching_peaks(
        ...
        self.decoys = np.array([idToDecoyDict[x] for x in self.libraryTags])

    def sort_matches_by_tags(self):
        self.libraryTags, self.queryTags, self.libraryIntensities, self.queryIntensities, self.ppmMatches = [
            np.array(x)
            for x in [self.libraryTags, self.queryTags, self.libraryIntensities, self.queryIntensities, self.ppmMatches]
        ]
        ...
        self.libraryTags, self.libraryIntensities, self.queryTags, self.queryIntensities, self.ppmMatches = [
            x[i1]
            for x in [self.libraryTags, self.libraryIntensities, self.queryTags, self.queryIntensities, self.ppmMatches]
        ]
