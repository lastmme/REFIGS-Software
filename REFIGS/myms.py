...
from . import sparse_nnls
from CsoDIAq import spectra_matcher_functions as smf


def get_scanNum_per_cycle(path: str):
    MS1 = []
    ...
    with mzxml.read(path, use_index=True) as spectra:
        for i, spec in enumerate(spectra, 1):
            if spec['msLevel'] == 2:
                ...
                mz, intensity = spec['m/z array'], spec['intensity array']
                window_info = spec['precursorMz'][0]
                central_mz, width = window_info['precursorMz'], window_info['windowWideness']
                # retentionTime=float(spec['retentionTime'])
                MS2.append([
                    np.concatenate(
                        (mz.reshape(-1, 1), intensity.reshape(-1, 1)), axis=1),  # peaks
                    (central_mz - width / 2, central_mz + width / 2),  # window
                    float(spec['retentionTime']),# RT
                    i  # scan number
                ])
    return MS2
    ...
    for charge in charges:
        ...
        while i < len(key_premz) and j < len(key_premz):
            if (i+1) % 1000 == 0:
                text_queue.put(
                    smf.print_milestone(
                        str(i + 1)+"/"+str(len(LibraryDividedByCharge))
                    )
                )
...
def spectrum_normalization(library: dict[str, dict]):
    for key, metadata in library.items():
        spc = metadata['Spectrum']
        # peaks intensity normalization
        spc[:, 1] = spc[:, 1]/np.sum(spc[:, 1])
        library[key]['Spectrum'] = spc
